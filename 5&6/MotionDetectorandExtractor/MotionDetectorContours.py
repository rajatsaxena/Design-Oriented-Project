...
def scatterplot(x,y):
    ...
    pyplot.ylabel('Avg. of Contour Area')
    pyplot.show()
...
class MotionDetectorAdaptative():
	...
	self.average = []
        ...
        self.capture=cv.CaptureFromFile('output.mp4')
    ...
    def initRecorder(self): #Create the recorder
        ...
        self.writer=cv.CreateVideoWriter(datetime.now().strftime("%b-%d_%H_%M_%S")+".wmv", codec, 10, cv.GetSize(self.frame), 1)
        #FPS set to 10
...
if __name__=="__main__":
    ...
    xindex = range(len(detect.average))
    scatterplot(xindex, detect.average)
    #print detect.average, xindex
